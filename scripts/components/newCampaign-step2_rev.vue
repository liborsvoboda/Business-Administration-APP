                ...
                <div class="row justify-content-center">
                    <div class="d-inline-flex">
                        <ul class="capturedUrl_campaign col-md-12 justify-content-center align-items-center">
                            ...
                            <li v-if="SelectedMessages.indexOf('Heureka') > -1" class="align-items-center pl-3 pr-3" style="display: inline-flex;background-color: transparent">
                                <span>
                                    <img src="/images/product/reviews.svg" class="svg_img_60" style="margin-right:10px;cursor:default;" />
                                </span>
                                <span class="with_100_points" style="vertical-align: top;">
                                    <div class="" style="vertical-align: top;font-weight: bold;">
                                        <span class="sub_title review_color">Recenze Heuréka</span>
                                    </div>
                                </span>
                            </li>
                        ...
                        <div class="row justify-content-md-center">
                            <div class="col-md-8">
                                <div class="list-group mt-2 shadow-sm">
                                    <div class="list-group-item group_item_blue1">
                                        <div class="new_campaign_tile_group_item review_color">Vyberte API klíč pro recenze a hodnocení</div>
                                    </div>

                                    <div class="list-group-item pt-4 pb-4">
                                        <div class="align-items-center" style="display: flex;">
                                            Vyberte HEUREKA API Klíč:
                                ...
                                v-tooltip.top="{ content: 'Automaticky = doplní samo správné znění odkazu včetně http:// nebo https:// k zadané URL.\n\nPřesná shoda = vložte přesnou URL stránky nebo podstránky (ideálně zkopírováním z URL řádku v prohlížeči).\n\nZačíná na = volba umožňuje zadat jen začátek URL a začlení všechny další znaky nebo parametry vypsané v URL (např. strom podstránek) ' }"></i>-->
                                            <select v-model="heurekaAPIkey" v-on:change="validateForm()" style="max-width: 240px;" name="heurekaAPIkey" v-validate="{required: true}" class="form-control ml-3" :class="{ 'is-invalid': errors.has('heurekaAPIkey') }">
                                                <option disabled="" value="none">Vyberte</option>
                                                <option v-for="apikey in apiKeyListHeureka" v-bind:value="apikey.heurekaAPIkey">
                                                    {{apikey.Name}}
                                                </option>
                                            </select>
                                        </div>
                                        <span v-if="errors.has('heurekaAPIkey')" class="invalid-feedback">{{ errors.first('heurekaAPIkey') }}</span>
                                        <!--<br v-if="SelectedMessages.indexOf('Heureka') > -1" />-->
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    export default {
        ...
        activated() {
            ...
            if (that.$store.state.CampaignData.campaignID != undefined) {
                ...
                if (that.heurekaAPIkey == 'none') { that.heurekaAPIkey = (that.$store.state.heurekaAPIkey != 'none') ? that.$store.state.heurekaAPIkey : (that.$store.state.CampaignData.heurekaAPIkey != "") ? that.$store.state.CampaignData.heurekaAPIkey : 'none'; }
            } else {
        ...
        methods: {
            ...
            goToStep(value) {
                this.$store.state.heurekaAPIkey = this.heurekaAPIkey;
            ...
            NextStep() {
                ...
                if (this.SelectedMessages.indexOf('Zbozi') > -1) {
                    this.Component = "app-new-campaign-Step2_zbozi";
                } else if (this.SelectedMessages.indexOf('OwnAppi') > -1) {
                    this.Component = "app-new-campaign-Step2_ownAppi";
                } else {
            ...
            validateForm() {
                ...
                this.$validator.validateAll({
                    heurekaAPIkey: this.heurekaAPIkey
				}).then((valid) => {
                    if (valid) {
                        var that = this;

                        if (that.heurekaAPIkey == 'none' && that.SelectedMessages.indexOf("Heureka") > -1) {
                            that.errors.add({ field: 'heurekaAPIkey', msg: 'Vyberte API klíč' });
                            that.nextStepAllowed2 = false;
                        }
                    } else {
                        if (this.heurekaAPIkey == 'none' && this.SelectedMessages.indexOf("Heureka") > -1) {
                            this.errors.add({ field: 'heurekaAPIkey', msg: 'Vyberte API klíč' });
