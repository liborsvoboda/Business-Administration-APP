                ...
                <div class="row justify-content-center">
                    <div class="d-inline-flex">
                        <ul class="capturedUrl_campaign col-md-12 justify-content-center align-items-center">
                            <li v-if="SelectedMessages.indexOf('LastEventsOrders') > -1" class="align-items-center pl-3 pr-3" style="display: inline-flex;background-color: transparent">
                                <span>
                                    <img src="/images/product/orders.svg" class="svg_img_60" style="margin-right:10px;cursor:default;" />
                                </span>
                                <span class="with_100_points" style="vertical-align: top;">
                                    <div class="" style="vertical-align: top;font-weight: bold;">
                                        <span class="sub_title order_color">Poslední objednávky</span>
                                    </div>
                                </span>
                            </li>
                        ...
                        <div class="row justify-content-md-center">
                            <div class="col-md-8">
                                <div class="list-group mt-2 shadow-sm">
                                    <div class="list-group-item group_item_purple">
                                        <div class="new_campaign_tile_group_item order_color d-flex align-items-center">
                                            <span class="width_100">Zadejte URL pro Poslední objednávky</span>
                                            <span class="right d-flex align-items-center">
                                                <button class="btn btn-outline-secondary_order fas fa-copy copyPaste" style="padding: 0.2rem 0.5rem;" v-on:click="copyToClipboard" title="Zkopírovat pravidla z této stránky" alt="Zkopírovat pravidla z této stránky"> </button>
                                                <button class="btn btn-outline-secondary_order fas fa-paste copyPaste" style="padding: 0.2rem 0.5rem;" v-on:click="pasteFromClipboard" title="Vložit zkopírovaná pravidla" alt="Vložit zkopírovaná pravidla" :disabled="!pasteEnable"> </button>
                                            </span>
                                        </div>
                                    </div>


                                    <div class="list-group-item" v-for="rule,index in CaptureRuleObj">
                                        <p v-if="index == 0">
                                        ...
                                        <div class="align-items-center pb-4 pt-3" style="display: flex;">
                                            <span class="d-flex width_100 align-items-center">
                                                ...
                                                <select v-model="CaptureRuleObj[index]" style="max-width: 240px;" v-on:change="validateForm()" v-bind:name="CaptureRuleObj[index]" v-validate="{required: true}" class="form-control" v-bind:class="{ 'is-invalid': errors.has('CaptureRuleObj['+index+']') }">
                                                    <option disabled="" value="none">Vyberte</option>
                                                    <option value="startWith" :hidden="CaptureRuleObj.indexOf('startWith') > -1 && CaptureRuleObj.indexOf('startWith') != index">Začíná na</option>
                                                    <option value="exactmatch" :hidden="CaptureRuleObj.indexOf('exactmatch') > -1 && CaptureRuleObj.indexOf('exactmatch') != index">Shoduje se</option>
                                                    <option value="contains" :hidden="CaptureRuleObj.indexOf('contains') > -1 && CaptureRuleObj.indexOf('contains') != index">Obsahuje</option>
                                                </select>
                                        ...
                                        <div class="d-flex">
                                            ...
                                            <tags-input :selectedRule="CaptureRuleObj[index]" v-bind:element-id="'tagsCapturedUrlsObjIn'+index" v-on:input="validateForm()" v-bind:ref="'tagsCapturedUrlsObjIn'+index" v-model="CapturedUrlsObjIn[index]" :allow-duplicates="false" placeholder="" v-bind:name="CapturedUrlsObjIn[index]" v-validate="{ required: false,url:require_protocol}" class="form-control" v-bind:class="{ 'is-invalid': errors.has('CapturedUrlsObjIn['+index+']') }" style="display: inline-table;table-layout: fixed;width: calc(100% - 70px);"></tags-input>
                                        </div>

                                        <div class="d-flex">
                                            <span class="width_70px">Vyloučit </span>
                                            <tags-input :selectedRule="CaptureRuleObj[index]" v-bind:element-id="'tagsCapturedUrlsObjOut'+index" v-on:input="validateForm()" v-bind:ref="'tagsCapturedUrlsObjOut'+index" v-model="CapturedUrlsObjOut[index]" :allow-duplicates="false" placeholder="" v-bind:name="CapturedUrlsObjOut[index]" v-validate="{ required: false,url:require_protocol}" class="form-control" v-bind:class="{ 'is-invalid': errors.has('CapturedUrlsObjOut['+index+']') }" style="display: inline-table;table-layout: fixed;width: calc(100% - 70px);"></tags-input>
                                        </div>
                                        <span v-if="errors.has('CapturedUrlsObjIn['+index+']') || errors.has('CapturedUrlsObjOut['+index+']') " class="d-flex invalid-feedback pl_70px">{{ errors.first('CapturedUrlsObjIn['+index+']') }} {{ errors.first('CapturedUrlsObjOut['+index+']') }}</span>
                                        <!--<span style="padding-top: 10px;display: block;padding-bottom: 10px;overflow-wrap: break-word;">{{CapturedUrlsObjIn[rule]}}</span>-->
                                    ...
                                    <div class="list-group-item pt-4 pb-4">
                                        <button class="btn btn-outline-secondary" v-on:click="addRule" :disabled="(!nextStepAllowed2 || CaptureRuleObj.length ==3)"><i class="fas fa-plus"></i>Přidat pravidlo</button>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    export default {
        ...
        activated() {
            ...
            if (that.$store.state.CampaignData.campaignID != undefined) {
                ...
                that.CaptureRuleObj = (that.$store.state.CaptureRuleObj[0] != 'none' || that.$store.state.CampaignData.CaptureRuleObj[0] == undefined) ? JSON.parse(JSON.stringify(that.$store.state.CaptureRuleObj)) : JSON.parse(JSON.stringify(that.$store.state.CampaignData.CaptureRuleObj)); 
                that.CapturedUrlsObjIn = (that.$store.state.CaptureRuleObj[0] != 'none' || that.$store.state.CampaignData.CaptureRuleObj[0] == undefined) ? JSON.parse(JSON.stringify(that.$store.state.CapturedUrlsObjIn)) : JSON.parse(JSON.stringify(that.$store.state.CampaignData.CapturedUrlsObjIn).replace(/""/g, '[]'));
                that.CapturedUrlsObjOut = (that.$store.state.CaptureRuleObj[0] != 'none' || that.$store.state.CampaignData.CaptureRuleObj[0] == undefined) ? JSON.parse(JSON.stringify(that.$store.state.CapturedUrlsObjOut)) : JSON.parse(JSON.stringify(that.$store.state.CampaignData.CapturedUrlsObjOut).replace(/""/g, '[]')); 
            } else {
        ...
        methods: {
            goToStep(value) {
                this.$store.state.CaptureRuleObj = JSON.parse(JSON.stringify(this.CaptureRuleObj));
                this.$store.state.CapturedUrlsObjIn = JSON.parse(JSON.stringify(this.CapturedUrlsObjIn));
                this.$store.state.CapturedUrlsObjOut = JSON.parse(JSON.stringify(this.CapturedUrlsObjOut));
            ...
            copyToClipboard() {
                this.$store.state.clipBoardRules = JSON.parse(JSON.stringify(this.CaptureRuleObj));
                this.$store.state.clipBoardUrlsIn = JSON.parse(JSON.stringify(this.CapturedUrlsObjIn));
                this.$store.state.clipBoardUrlsOut = JSON.parse(JSON.stringify(this.CapturedUrlsObjOut));
                ...
                this.$dialogs.confirm('Chcete opravdu vložit pravidla z dočasné schránky ? Přepíší se již zadané URL adresy na této stránce.', options)
                    .then(res => {
                        if (res.ok) {
                            ...
                            this.CaptureRuleObj = JSON.parse(JSON.stringify(this.$store.state.clipBoardRules));
                            this.CapturedUrlsObjIn = JSON.parse(JSON.stringify(this.$store.state.clipBoardUrlsIn));
                            this.CapturedUrlsObjOut = JSON.parse(JSON.stringify(this.$store.state.clipBoardUrlsOut));
            ...
            addRule() {
                this.validatingEnabled = false;
                this.CaptureRuleObj.push('none');
                this.CapturedUrlsObjIn.push([]);
                this.CapturedUrlsObjOut.push([]);
                this.validatingEnabled = true;
                this.validateForm();
            },
            removeRule(value) {
                this.validatingEnabled = false;
                this.CaptureRuleObj.splice(value, 1);
                this.CapturedUrlsObjIn.splice(value, 1);
                this.CapturedUrlsObjOut.splice(value, 1);
                this.validatingEnabled = true;
                this.validateForm();
            },
            goToSelected(value) {
            ...
            NextStep() {
                ...
                if (this.SelectedMessages.indexOf('LastEventsRegistrations') > -1) {
                    this.Component = "app-new-campaign-Step2_reg";
                } else if (this.SelectedMessages.indexOf('VisitCount') > -1) {
                    this.Component = "app-new-campaign-Step2_visit";
                } else if (this.SelectedMessages.indexOf('Statistics') > -1) {
            ...
            validateForm() {
                ...
                this.$validator.validateAll({
                    CapturedUrlsObjIn: this.CapturedUrlsObjIn,
                    CaptureRuleObj: this.CaptureRuleObj
                }).then((valid) => {
                    if (this.validatingEnabled) {
                        if (valid) {
                            that.CaptureRuleObj.forEach(function (rule, index) {
                                if (rule == 'none') {
                                    that.errors.add({ field: 'CaptureRuleObj[' + index + ']', msg: 'Vyberte variantu' });
                            ...
                            that.CapturedUrlsObjIn.forEach(function (ruleUrlsIn, index) {
                                ...
                                if (index > 0) {
                                    ...
                                    if (checkInsertedMinimumUrl.length == 0) {
                                        that.errors.add({ field: 'CapturedUrlsObjIn[' + index + ']', msg: 'Musí být zadána minimálně 1 URL.' });
                                        that.errors.add({ field: 'CapturedUrlsObjOut[' + index + ']', msg: ' ' });
                                        that.nextStepAllowed2 = false;
                                    }
                                }
                            });
                            ...
                            if (that.CapturedUrlsObjIn[0].length == 0) {
                                that.errors.add({ field: 'CapturedUrlsObjIn[0]', msg: 'Není zadaná žádná zahrnutá URL.' });
                                that.nextStepAllowed2 = false;
                            }
                                ...
                                if (ruleUrlsIn.length > 0) {
                                    ruleUrlsIn.forEach(function (urlAddressIn) {
                                        if (urlAddressIn != '' && !that.validUrl(urlAddressIn) && that.CaptureRuleObj[index] != 'contains') {
                                            that.errors.add({ field: 'CapturedUrlsObjIn[' + index + ']', msg: 'URL adresa nemá správný formát.' });
                                            that.nextStepAllowed2 = false;
                                        }
                                    });
                                }
                                ...
                                if (that.CapturedUrlsObjOut[index].length > 0) {
                                    that.CapturedUrlsObjOut[index].forEach(function (urlAddressOut) {
                                        if (urlAddressOut != '' && !that.validUrl(urlAddressOut) && that.CaptureRuleObj[index] != 'contains') {
                                            that.errors.add({ field: 'CapturedUrlsObjOut[' + index + ']', msg: 'URL adresa nemá správný formát.' });
                                            that.nextStepAllowed2 = false;
                                        }
                                    });
                                }
                                        ...
                                        if (urlAddressIn != null) {
                                            ...
                                            if (ruleUrlsList.length > 0 && !that.ignoreDomain) {
                                                if ((
                                                    ...
                                                    that.errors.add({ field: 'CapturedUrlsObjIn[' + index + ']', msg: 'V Kampani může být nastavena jen jedna doména: ' + that.selectedDomain });
                                                    that.nextStepAllowed2 = false;
                                                }
                                            }
                                        }
                                        ...
                                        if (urlAddressOut != null) {
                                            ...
                                            if (ruleUrlsList.length > 0 && !that.ignoreDomain) {
                                                if ((
                                                    ...
                                                    that.errors.add({ field: 'CapturedUrlsObjOut[' + index + ']', msg: 'V Kampani může být nastavena jen jedna doména: ' + that.selectedDomain });
                                                    that.nextStepAllowed2 = false;
                                                }
                                            }
                                        }
